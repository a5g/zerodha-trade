...
function headerInfo() {
  const arr = []
  arr[0] = `\n\nGTT Orders for below instruments have been created for [${utils.kiteuser().name}] [kcid: ${utils.kiteuser().kcid}] [capital: ${utils.kiteuser().capital.toLocaleString('en-IN')}]`
  arr[1] = DIVIDER
  arr[2] =
    utils.pad('No.', 5) +
