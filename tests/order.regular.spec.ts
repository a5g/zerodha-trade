...
function headerInfo() {
  const arr = []
  arr[0] = `\n\nBelow REGULAR orders have been executed for [${utils.kiteuser().name}] [kcid: ${utils.kiteuser().kcid}] [capital: ${utils.kiteuser().capital.toLocaleString('en-IN')}]`
  arr[1] = DIVIDER
  arr[2] =
    utils.pad('No.', 5) +
    utils.pad('Stock', 15) +
...
test.describe(`Regular`, () => {
  ...
  test.afterAll(() => {
    const arr = []
    arr[0] = `\n\nBelow Regular cancel orders have been executed for [${utils.kiteuser().name}] [kcid: ${utils.kiteuser().kcid}]`
    arr[1] = '----------------------'
